...
export class ImBridge {
  static async create(ctx, cfg) {
    ...
    await dsh.init().catch((e) => {
      log.error("DSH 会话管理器初始化失败，QQ 桥接停用：", e.message);
      throw e;
    });
  ...
  async _sendFile(chat, absPath) {
    if (chat.platform === "qq") {
      try {
        ...
        if (chat.isGroup) {
          return (await this.qq.uploadGroupFile(chat.chatId, absPath, name)) ||
            (await this.qq.sendGroupFileSegment(chat.chatId, absPath));
        }
        return (await this.qq.uploadPrivateFile(chat.chatId, absPath, name)) ||
