...
import { transcribeAudio, warmupAsr } from "./asr.js";

// 调试日志（写文件，避免与 NapCat/DSH 日志混淆）
const DBG_LOG = join(process.env.DSH_HOME || join(process.env.USERPROFILE || "C:/Users/ZJL", ".dsh"), "im-bridge", "state", "debug.log");
function dbg(msg) {
  try {
...
export class ImBridge {
  static async create(ctx, cfg) {
    ...
    // 预热语音识别常驻服务（后台加载模型，第一条语音也不用等模型）
    const asrCfg = cfg.asr || {};
    if (asrCfg.enabled !== false) {
      warmupAsr({
        python: asrCfg.python || "py",
        model: asrCfg.model || "small",
      }).then((ok) => log.info(`ASR 预热: ${ok ? "就绪" : "失败(将按需启动)"}`)).catch(() => {});
    }
    return bridge;
  }
  ...
  async handleQqEvent(ev) {
    dbg(`QQ event: post=${ev.post_type} type=${ev.message_type} segs=${JSON.stringify(Array.isArray(ev.message) ? ev.message.map((s) => s.type) : typeof ev.message)}`);
    ...
    for (const s of segments) {
      switch (s.type) {
        ...
        case "record":
          parts.push("[语音]");
          // NapCat 的 record 段: file=文件名(相对), path=本地绝对路径, url=下载地址
          dbg(`QQ record data: ${JSON.stringify(s.data)}`);
          inbound.push({
            kind: "voice",
            url: s.data?.url,
            localPath: s.data?.path || s.data?.file,
            name: s.data?.file || "voice",
          });
          break;
        default:
  ...
  async _processChat(chat, pcfg) {
    ...
    for (const f of chat.inbound || []) {
      dbg(`materialize ${f.kind}: url=${f.url ? "yes" : "no"} local=${f.localPath || "none"}`);
      const saved = await this._materializeInbound(chat, mapping, f);
      dbg(`materialize ${f.kind} result: ${saved ? saved.path : "FAILED"}`);
      if (saved) {
        // 语音：识别后即删，不加入文件列表（用户要求不保存语音）
        if (f.kind === "voice") {
          const asrCfg = this.cfg.asr || {};
          const text = await transcribeAudio(saved.path, {
            python: asrCfg.python || "py",
            model: asrCfg.model || "small",
            lang: asrCfg.lang || "zh",
            timeoutMs: asrCfg.timeoutMs || 180000,
          });
          if (text) asrLines.push(`用户发来一条语音，识别内容为：“${text}”`);
          else asrLines.push("（用户发来一条语音，识别失败或无文字内容）");
          // 清理本地语音文件：删 amr 副本及转码产物
          for (const p of [saved.path, `${saved.path}.wav`, `${saved.path}.pcm`]) {
            try { await unlink(p); } catch { /* ignore */ }
          }
          dbg("voice cleaned up (files deleted)");
        } else {
          files.push(saved);
        }
      }
    ...
    if (asrLines.length > 0) lines.push(asrLines.join("\n"));
    if (files.length > 0) {
      lines.push("用户发来了文件：");
  ...
  async _materializeInbound(chat, mapping, f) {
    ...
    if (chat.platform === "qq") {
      ...
      if (saved) return { path: saved, name: f.name || "file", kind: f.kind };
  ...
  async _sendFile(chat, absPath) {
    if (chat.platform === "qq") {
      const name = safeBase(absPath);
      // 优先使用 file 消息段：实测 upload_private_file 在部分 QQ 环境下返回
      // 成功（file_id）但文件实际未送达（rich media transfer failed），
      // 而 file 消息段（send_private_msg + file 段）更可靠。
      const attempts = chat.isGroup
        ? [
            () => this.qq.sendGroupFileSegment(chat.chatId, absPath),
            () => this.qq.uploadGroupFile(chat.chatId, absPath, name),
          ]
        : [
            () => this.qq.sendPrivateFileSegment(chat.chatId, absPath),
            () => this.qq.uploadPrivateFile(chat.chatId, absPath, name),
          ];
