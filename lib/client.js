/* dsh-qqbot —— 客户端半：QQ 机器人控制台面板
 * 打包格式：window.__ModuleLoader__.load({ id, factory })。
 * 功能：
 *   - 设置页「QQ 机器人控制台」区块：NapCat 启停 / 状态 / 登录二维码 / 重启 DSH
 *   - 右下角悬浮按钮，随时展开同一个面板
 * 数据来自宿主路由 /api/qqbot/*。
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var COLORS = {
      ...
      bg: "var(--dsw-alias-bg-layer-1, rgba(128,128,128,0.07))",
      overlay: "var(--dsw-alias-bg-overlay, #fff)",
      btn: "var(--dsw-alias-button-elevated-fill, rgba(127,127,127,0.15))",
    };
    var BTN = {
      display: "inline-flex", alignItems: "center", gap: 6,
      padding: "6px 14px", borderRadius: 8, border: "1px solid " + COLORS.border,
      background: COLORS.btn, color: COLORS.label, fontSize: 13, cursor: "pointer",
    ...
    function Panel(props) {
      ...
      var refresh = useCallback(function () {
        fetch("/api/qqbot/status", { cache: "no-store" })
      ...
      // 未登录时每 5 秒刷新二维码图片（避免显示过期二维码）
      useEffect(function () {
        var nap = st && st.napcat;
        if (!nap || nap.loggedIn || !nap.qrcode) return;
        var t = setInterval(function () { setTick(function (x) { return x + 1; }); }, 5000);
        return function () { clearInterval(t); };
      }, [st]);
      ...
      function restartDsh() {
        ...
        act("restart", "/api/qqbot/restart-dsh");
      }
      ...
      return h("div", { style: { fontFamily: "inherit" } },
        ...
        h("div", { style: CARD },
          ...
          h("div", { style: Object.assign({}, ROW, { marginTop: 10 }) },
            h("button", { style: BTN_PRIMARY, disabled: !!busy || nap.running, onClick: function () { act("start", "/api/qqbot/napcat/start"); } }, busy === "start" ? "启动中…" : "▶ 启动 NapCat"),
            h("button", { style: BTN_DANGER, disabled: !!busy || !nap.running, onClick: function () { act("stop", "/api/qqbot/napcat/stop"); } }, busy === "stop" ? "停止中…" : "■ 停止 NapCat"),
            nap.webuiUrl
        ...
        notLoggedIn && nap.qrcode
          ? h("div", { style: Object.assign({}, CARD, { marginTop: 10, textAlign: "center" }) },
              ...
              h("img", {
                src: "/api/qqbot/napcat/qrcode?t=" + Date.now() + Math.floor(Math.random() * 1000),
    ...
    function FloatingLauncher() {
      ...
      return h("div", { style: { position: "fixed", right: 20, bottom: 20, zIndex: 9999, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 } },
        open
          ? h("div", { style: { width: 340, maxHeight: "70vh", overflow: "auto", background: COLORS.overlay, borderRadius: 12, boxShadow: "0 8px 30px rgba(0,0,0,0.18)", border: "1px solid " + COLORS.border, padding: 14 } },
              h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 } },
                h("span", { style: { fontSize: 14, fontWeight: 600, color: COLORS.label } }, "QQ 机器人控制台"),
    ...
    function SettingsSection() {
      return h("div", { style: { maxWidth: 640 } },
        ...
        h("div", { style: { fontSize: 11, color: COLORS.warn, marginTop: 6, lineHeight: 1.5, borderTop: "1px solid " + COLORS.border, paddingTop: 8 } },
          "⚠️ 免责声明：本插件通过非官方协议（NapCat）接入 QQ，存在封号风险，请务必使用小号；仅供学习交流，禁止用于违法违规用途（详见插件 DISCLAIMER.md）。")
      );
    }
      ...
      ctx.effect(function () {
        return slots.inject("shell.overlay", function () {
          return slots.register(
            { name: "shell.overlay", id: "dsh-qqbot-fab", order: 200, label: "QQ Bot" },
            FloatingLauncher
          );
        });
      }, "dsh-qqbot: shell.overlay");
      ctx.effect(function () {
        return slots.inject("settings.section", function () {
          return slots.register(
            { name: "settings.section", id: "dsh-qqbot-panel", order: 80, label: () => "QQ 机器人控制台" },
            SettingsSection
          );
        });
      }, "dsh-qqbot: settings.section");
    }
