/**
 * dsh-qqbot 控制台：通过 DSH Web 服务的 HTTP 路由暴露 NapCat 启停 / 状态 / 二维码，
 * 供浏览器端控制面板调用。
 *
 * 路由：
 *   GET  /api/qqbot/status          状态（NapCat 进程/端口/登录态/二维码/桥接连接）
 *   POST /api/qqbot/napcat/start    启动 NapCat（后台分离进程）
 *   POST /api/qqbot/napcat/stop     停止 NapCat
 *   GET  /api/qqbot/napcat/qrcode   返回当前登录二维码 PNG
 *   POST /api/qqbot/restart-dsh     重启 DSH Web（延迟 3 秒，实验性）
 */
